#include <linux/init.h>
 ...
 asmlinkage long sys_mycall0(long);//自定义的系统调用函数
 ...
 asmlinkage long sys_mycall0(long num)
 {
...
#define BASE_ADDR 0xffff880000000000
...
uint8_t temp = 0; /* Used so compiler won’t optimize out victim_function() */
 ...
 {
     volatile int z;
     //printk("Spectre: This is my system call! so = 0x%lx, probo = 0x%lx\n", (unsigned long)secret_offset, (unsigned long)prob_offset);
     array2 = array2_temp + array1[secret_offset] * 4096 + prob_offset; // shared mem = array2
     for (z = 0; z < 100; z++) {}
...
static size_t sys_mycall2(size_t phys_address) {
    void * vir_address;
    printk("Spectre: This is my syscall 2, pa = %lx\n", (unsigned long)phys_address);
    if (phys_address == 1) {
